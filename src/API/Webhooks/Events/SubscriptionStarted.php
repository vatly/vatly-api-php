 ...
 *
 * Built by the webhook event factory via a follow-up `GetSubscription` call
 * against the webhook's `entityId` (carrying the mandate summary). On a
 * transient API failure it falls back to the webhook payload, which itself
 * embeds the mandate inline — so the fallback stays non-lossy.
 *
 * @immutable
 */
