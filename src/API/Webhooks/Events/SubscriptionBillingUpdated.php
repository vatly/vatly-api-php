...
use Vatly\API\Webhooks\Concerns\ParsesWebhookMandate;
 ...
 * mandate otherwise changes server-side). Built by the webhook event factory
 * via a follow-up `GetSubscription` call against the webhook's `entityId` (the
 * fresh mandate summary). On a transient API failure it falls back to the
 * webhook payload, which embeds the mandate inline — so the fallback stays
 * non-lossy.
 *
...
class SubscriptionBillingUpdated
{
    ...
    public function __construct(
         ...
         * should leave the stored mandate untouched rather than wiping it. The
         * mandate's method + maskedIdentifier are atomically bound together
         * inside the Mandate object so listeners can't mix old + new values.
         */
    ...
    /**
     * Build from the enriched API resource fetched by the webhook event factory.
     */
