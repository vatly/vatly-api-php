 ...
 * This direct event lets local state flip atomically and removes a class of
 * "the cron is late so the state is wrong" bugs.
 *
 * A grace-period-completed reaction stamps the actual `endsAt` onto the local
 * row. In the happy path the cancellation already stamped the scheduled end so
 * this is an idempotent re-write, but it self-heals a missed/out-of-order
